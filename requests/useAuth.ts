...
export const useAuth = () => {
  ...
  const { setUser } = usePersistentStore();
  ...
  const emailLogin = async (postData: any) => {
    try {
      ...
      const { data } = await request.post(
        `emailLoginVerify?email=${encodeURIComponent(
          postData.email
        )}&token=${encodeURIComponent(postData.token)}`,
        { a: 1, b: 2, c: 3, o: 4 }
      );
      setLoading(false);
      if (data.id) {
  ...
  const sendVerificationEmail = async () => {
    try {
      ...
      if (!email0) return;
      await request.post(`emailLogin?email=${encodeURIComponent(email0)}`, {
        a: 1,
        b: 2,
        c: 3,
        o: 5,
      ...
      setLoading(false);
      toast.info(
        "Verfication email sent, please check your email inbox and use the mentioned link to login.",
        {
          position: toast.POSITION.TOP_RIGHT,
        }
      );
